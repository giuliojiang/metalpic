    ...
    static createApp() {
        ...
        app.get("/:albumname/:page", async (req, res) => {
            try {
                ...
                let dom = dom_utils_1.DomUtils.createNewDocument(`/metalpic-album/${encodeURIComponent(req.params.albumname)}/${page}`);
                // Get the album
                let albumAndPictures = await routeAlbum.getAlbumAndPictures(req.params.albumname, page, false);
                // Write album name
                dom_utils_1.DomUtils.addH1(dom, albumAndPictures.album.name);
                // Set title
                dom_utils_1.DomUtils.setTitle(dom, `metalpic - ${albumAndPictures.album.name}`);
                // Write album pictures
                for (let pic of albumAndPictures.pictures) {
                    dom_utils_1.DomUtils.addText(dom, pic.name);
